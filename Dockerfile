...
# Ferramentas de build e dev-libs (inclui Boost, Event, OpenSSL e **GMP**)
RUN apt-get update && apt-get install -y \
  build-essential libtool autotools-dev automake pkg-config \
  libssl-dev libevent-dev libboost-all-dev \
  libgmp-dev wget curl git ca-certificates m4 \
...
# Autotools + flags do DB4 e do GMP (garante detecção no link)
RUN ./autogen.sh && \
    ./configure \
